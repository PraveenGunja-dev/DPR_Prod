...
export interface P6Activity {
    ...
    uom: string;
    blockCapacity: string;  // Block Capacity (MWac)
    phase: string;           // Phase
    spvNo: string;           // SPV Number (e.g., ASEJ6PL)
    scope: string;
    hold: string;            // Hold status
    front: string;
...
/**
 * P6 activities mapped for DP Block table (18 columns)
 */
export const mapActivitiesToDPBlock = (activities: P6Activity[]) => {
    return activities.map((activity) => ({
        // Identification
        activityId: activity.activityId,
        activities: activity.activities || activity.description,

        // Block details from P6 UDF
        blockCapacity: activity.blockCapacity || activity.totalQuantity || '',
        phase: activity.phase || '',
        block: activity.block || activity.newBlockNom || '',
        spvNumber: activity.spvNo || activity.plot || '', // SPV Number (e.g., ASEJ6PL)

        // Status fields
        priority: activity.priority || activity.baselinePriority || '',
        scope: activity.scope || '',
        hold: activity.hold || activity.holdDueToWtg || '',
        front: activity.front || '',
        completed: activity.actual || activity.completionPercentage || '',
        balance: activity.balance || '',
