...
<tr><td style="line-height:0;background:#09090b;"><img src="{base_url}/coverPhoto.png" width="600" alt="Cover" style="display:block;width:100%;max-width:600px;height:auto;"></td></tr>
<tr><td style="padding:30px 40px 20px;border-bottom:1px solid #f0f0f0;">
<table width="100%"><tr><td>
<h1 style="color:#09090b;margin:0;font-size:24px;font-weight:700;">{title}</h1>
<p style="color:#64748b;margin:6px 0 0;font-size:14px;">{subtitle}</p>
</td><td align="right" valign="top"><div style="font-weight:800;font-size:18px;color:#09090b;">Digitalized DPR</div></td></tr></table>
</td></tr>
...
async def _send_mail(to: str, subject: str, html: str) -> dict:
    ...
    try:
        ...
        await aiosmtplib.send(
            ...
            port=smtp_port,
            use_tls=use_tls,
            start_tls=start_tls if smtp_port not in (25,) else False,
...
async def send_access_approved_email(user_email: str, user_name: str, assigned_role: str) -> dict:
    ...
    <div style="background:#ecfdf5;border-radius:8px;border:1px solid #10b981;padding:20px;margin-bottom:30px;">
      <p style="margin:0 0 4px;color:#064e3b;font-weight:600;">ASSIGNED ROLE</p>
      <span style="background:#10b981;color:#fff;padding:4px 12px;border-radius:20px;font-size:13px;">{assigned_role}</span>
    </div>
