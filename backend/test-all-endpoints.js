// Explore what endpoints are available in P6 API
require('dotenv').config();

const axios = require('axios');

const baseUrl = 'https://sin1.p6.oraclecloud.com/adani/stage/p6ws/restapi';
const token = 'eyJ4NXQjUzI1NiI6IlV6LU1BTlgyS0VncEFpb2I3cEVwQlZWSmtZSzFvV2FRczBacHhMbDI5NWciLCJ4NXQiOiJGNmE4X1lJMENCTEI3LVpkd3RWNjM5bXFqZ0kiLCJraWQiOiJTSUdOSU5HX0tFWSIsImFsZyI6IlJTMjU2In0..di4pkFvxg-F_QMfnqCSwIvdQ7kNljaDGi6IzOf2wJsMVpaIOLg05zg30c2t3uz2SLur-24AMx8jEgquoyQbAYz-Zw5hn_loNgRPMChvTFAJmuo_GCnNHXzItEfRmgssmYj-M-z0xnCPSI6Vexc4OfXNP2KUOLZNL8Mfqt2Slp9rcWhizHosWHdStiZNXalYpg-P0UU1fs3Gbn8YAI80vRlTLtgKK-qzGYQuogvloqeOodZ38g-_A46hWZIspp6HDR2GZ3Scyz6MflPRz15ex1cptsAAibP2mD7zUs49SjPkZeJa-lBrTLoj4C7G-Tq3fdPdN7bgRwZcmX2Bs0_liKg';

const headers = {
    'Authorization': `Bearer ${token}`,
    'Accept': 'application/json'
};

// Common P6 REST API endpoints to test
const endpoints = [
    // Core endpoints (known working)
    '/project',
    '/activity',
    '/wbs',
    // Resource related
    '/resource',
    '/resourceassignment',
    // UDF related
    '/udftype',
    '/UDFType',
    '/udfvalue',
    '/UDFValue',
    '/udfcode',
    '/UDFCode',
    // Calendar/Schedule
    '/calendar',
    '/relationship',
    // EPS (Enterprise Project Structure)
    '/eps',
    // Other common
    '/activitycode',
    '/activitycodetype',
    '/activitycodeassignment',
    '/activitystep',
    '/spreadperiod',
    '/unitofmeasure'
];

async function testEndpoint(endpoint) {
    try {
        const response = await axios.get(`${baseUrl}${endpoint}`, {
            headers,
            params: { Fields: 'ObjectId' },
            timeout: 10000
        });
        const count = Array.isArray(response.data) ? response.data.length : 'obj';
        console.log(`✓ ${endpoint.padEnd(25)} -> ${response.status} (${count} items)`);
        return true;
    } catch (error) {
        console.log(`✗ ${endpoint.padEnd(25)} -> ${error.response?.status || 'ERR'}`);
        return false;
    }
}

(async () => {
    console.log('=== Scanning P6 REST API Available Endpoints ===\n');
    console.log('Base URL:', baseUrl);
    console.log('\n');

    const working = [];
    const notWorking = [];

    for (const endpoint of endpoints) {
        const success = await testEndpoint(endpoint);
        if (success) working.push(endpoint);
        else notWorking.push(endpoint);
    }

    console.log('\n=== Summary ===');
    console.log('\nWorking endpoints:', working);
    console.log('\nNot available (404):', notWorking.length, 'endpoints');
})();
